...
def render_layout(title, content, user=None):
    ...
    <style>
        ...
        .btn {{ background: #0f4573; color: white; border: none; padding: 10px 15px; cursor: pointer; border-radius: 4px; }}
...
def application(environ, start_response):
    ...
    if path == "/dashboard":
        ...
        start_response("200 OK", [("Content-Type", "text/html")]); return [render_layout("Dashboard", content, user_data).encode("utf-8")]

    # --- PERFILES ---
    if path == "/perfiles":
        conn = conectar_bd(); cur = conn.cursor(dictionary=True); cur.execute("SELECT * FROM perfiles"); perfiles = cur.fetchall(); cur.close(); conn.close()
        filas = "".join([f"<tr><td>{p['id']}</td><td>{p['strNombrePerfil']}</td><td>{'Admin' if p['bitAdministrador'] else 'User'}</td></tr>" for p in perfiles])
        content = f"<div class='card'><h3>Perfiles</h3><table><tr><th>ID</th><th>Nombre</th><th>Tipo</th></tr>{filas}</table><br><a href='/dashboard' class='btn'>Volver</a></div>"
        start_response("200 OK", [("Content-Type", "text/html")]); return [render_layout("Perfiles", content, user_data).encode("utf-8")]

    # --- USUARIOS ---
    if path == "/usuarios":
        conn = conectar_bd(); cur = conn.cursor(dictionary=True); cur.execute("SELECT u.*, p.strNombrePerfil FROM usuarios u LEFT JOIN perfiles p ON u.idPerfil = p.id"); users = cur.fetchall(); cur.close(); conn.close()
        filas = "".join([f"<tr><td>{u['strNombreUsuario']}</td><td>{u['strNombrePerfil']}</td><td>{u['strCorreo']}</td></tr>" for u in users])
        content = f"<div class='card'><h3>Usuarios</h3><table><tr><th>Usuario</th><th>Perfil</th><th>Correo</th></tr>{filas}</table><br><a href='/dashboard' class='btn'>Volver</a></div>"
        start_response("200 OK", [("Content-Type", "text/html")]); return [render_layout("Usuarios", content, user_data).encode("utf-8")]
