...
def render_layout(title, content, user=None):
    ...
    if user:
        ...
        # 1. Módulos con permiso de VER para este perfil
        ...
        # ── Módulos de la BD que pertenecen a Seguridad y tienen permiso ──
        # (por si el admin agrega módulos propios bajo Seguridad)
        for m in all_mods:
            if m['strMenuPadre'] == 'Seguridad' and m['strNombreModulo'].lower() in permitidos:
                ruta = m["strRuta"] or f"/{m['strNombreModulo'].lower().replace(' ', '-')}"
                seg_html += f'<a href="{ruta}">📦 {m["strNombreModulo"]}</a>'
 
        dropdown_seguridad = f"""
        <div class="dropdown">
            <button class="dropbtn">Seguridad ▾</button>
            <div class="dropdown-content">{seg_html}</div>
        </div>""" if seg_html else ""
 
        # ── Principal 1: módulos de la BD con ese padre y con permiso ──
        p1_html = ""
        for m in all_mods:
            if m['strMenuPadre'] == 'Principal 1' and m['strNombreModulo'].lower() in permitidos:
                ruta = m["strRuta"] or f"/{m['strNombreModulo'].lower().replace(' ', '-')}"
                p1_html += f'<a href="{ruta}">📦 {m["strNombreModulo"]}</a>'
 
        dropdown_p1 = f"""<div class="dropdown"><button class="dropbtn">Principal 1 ▾</button>
                          <div class="dropdown-content">{p1_html}</div></div>""" if p1_html else ""
 
        # ── Principal 2: igual ──
        p2_html = ""
        for m in all_mods:
            if m['strMenuPadre'] == 'Principal 2' and m['strNombreModulo'].lower() in permitidos:
                ruta = m["strRuta"] or f"/{m['strNombreModulo'].lower().replace(' ', '-')}"
                p2_html += f'<a href="{ruta}">📦 {m["strNombreModulo"]}</a>'
 
        dropdown_p2 = f"""<div class="dropdown"><button class="dropbtn">Principal 2 ▾</button>
                          <div class="dropdown-content">{p2_html}</div></div>""" if p2_html else ""
...
def application(environ, start_response):
    ...
    if path == "/api/get_permisos":
        ...
        if idp_raw:
            conn = conectar_bd(); cur = conn.cursor(dictionary=True)
            try:
                cur.execute("""SELECT nombreModulo,
                               permisoVer, permisoCrear,
    ...
    if path in ("/", "/dashboard"):
        ...
        <style>
            ...
            .welcome-card-hero {{
                ...
                color: white; border-radius: 24px; padding: 80px 40px;
            ...
            .welcome-badge {{
                ...
                font-weight: 600; margin-bottom: 30px; color: white;
                text-transform: uppercase; letter-spacing: 1px;
            }}
            .welcome-card-hero h1 {{
                font-size: 3.2rem; font-weight: 800; margin: 0 0 20px 0;
                text-transform: uppercase; letter-spacing: -1px; line-height: 1.1; color: white;
            }}
            .welcome-card-hero p {{
                font-size: 1.25rem; opacity: 0.95; max-width: 650px;
                margin: 0 auto; line-height: 1.6; font-weight: 300; color: #ecfdf5;
            }}
            .status-line {{
                margin-top: 50px; opacity: 0.8; font-size: 11px;
                letter-spacing: 2px; color: #d1fae5; text-transform: uppercase;
            }}
        </style>
        <div class="welcome-wrapper">
            <div class="welcome-card-hero">
                ...
                <div class="status-line">ESTADO DEL SISTEMA: OPERATIVO</div>
            </div>
        </div>
        """
        ...
        if not p_user['permisoVer']:
            ...
            <script>
              ...
              function validateAndSave() {{
                ...
                runCrud('save','usuarios',0,{{ u, p, c, t, 
              ...
              function updateUser() {{
                ...
                runCrud('update','usuarios', document.getElementById('ed_id').value, {{
                  u: u, idp: document.getElementById('ed_idp').value, st: document.getElementById('ed_st').value
                }});
              }}
            </script>"""
 
    # ----------------------------------------------------------
    # 8. PERFILES (CON VALIDACIÓN DE DUPLICADOS)
    # ----------------------------------------------------------
    elif path == "/perfiles":
        id_p = u_data.get('pid')
            ...
            <script>
              ...
              function savePerfil() {{
                ...
                if (perfilesExistentes.includes(n.toLowerCase()))
                    return alert("❌ Ya existe un perfil con el nombre '" + n + "'");
              ...
              function updatePerfil() {{
                ...
                if (!n) return alert("⚠️ El nombre es obligatorio");
                const nombreOriginal = document.querySelector('.p-row button[onclick*="' + id + '"]')
                    .parentElement.parentElement.querySelector('.p-name').innerText.trim().toLowerCase();
                if (n.toLowerCase() !== nombreOriginal && perfilesExistentes.includes(n.toLowerCase()))
                    return alert("❌ Ya existe otro perfil con el nombre '" + n + "'");
