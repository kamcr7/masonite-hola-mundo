...
JWT_SECRET = "CLAVE_MAESTRA_CLINICA_2026_FIX_V1"
...
def inicializar_datos():
    ...
    # Reparación de emergencia para el error 'Unknown column strRuta'
    try:
        cur.execute("SELECT strRuta FROM modulos LIMIT 1")
    except:
        cur.execute("ALTER TABLE modulos ADD COLUMN strRuta VARCHAR(100)")
        cur.execute("ALTER TABLE modulos ADD COLUMN strMenuPadre VARCHAR(50)")
        ...
        nav = f"""<div class="top-nav">
            <div class="nav-container">
                <div class="nav-left">
                    ...
                    <div class="dropdown"><button class="dropbtn">Principal 1 ▾</button><div class="dropdown-content">{get_links("Principal 1") or '<a>(Sin módulos)</a>'}</div></div>
                    <div class="dropdown"><button class="dropbtn">Principal 2 ▾</button><div class="dropdown-content">{get_links("Principal 2") or '<a>(Sin módulos)</a>'}</div></div>
                </div>
...
def application(environ, start_response):
    ...
    if path == "/modulos":
        ...
        rows = "".join([f"<tr><td>{u['strNombreUsuario']}</td><td>{u['strCorreo']}</td><td>{u['strNombrePerfil'] or 'Sin Perfil'}</td><td><button class='btn-red' onclick=\"runCrud('delete','usuarios',{u['id']})\">Eliminar</button></td></tr>" for u in cur.fetchall()])
        ...
        content = f"""<div class='card'><h2>👥 Usuarios</h2><button class='btn-emerald' onclick="openM('mU')">+ Nuevo Usuario</button>
            ...
            <button class="btn-emerald" style="width:100%" onclick="runCrud('save_usuario','usuarios',0,{{u:document.getElementById('uu').value, c:document.getElementById('ue').value, pwd:document.getElementById('up').value, p:document.getElementById('uperf').value}})">Crear</button></div></div>"""
        ...
        mods_manuales = [{'id': 991, 'n': 'Perfiles'}, {'id': 992, 'n': 'Módulos'}, {'id': 993, 'n': 'Usuarios'}, {'id': 994, 'n': 'Permisos'}]
        ...
        content = f"""<div class='card'><h2>🔐 Matriz de Permisos</h2><select onchange="location.href='?p='+this.value"><option value="0">-- Seleccionar Perfil --</option>{opts}</select>
            <button class="btn-emerald" onclick="alert('Guardado (Simulado)')">Guardar Cambios</button>
            <table><thead><tr><th>Módulo</th><th>Ver</th><th>Crear</th><th>Editar</th></tr></thead><tbody>{m_rows}</tbody></table></div>"""
        ...
        start_response("303 See Other", [("Location", "/"), ("Set-Cookie", "token=; Max-Age=0")]); return [b""]
    else:
        content = f"<h2>Panel Principal</h2><p>Bienvenido. Usa el menú de arriba para gestionar la clínica.</p>"
