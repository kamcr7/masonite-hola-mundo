...
def application(environ, start_response):
    ...
    if path in ["/", "/login"] and method == "GET":
        content = """<div class='card' style='max-width:350px; margin:100px auto; text-align:center;'>
            <h2>🛡️ Clínica Santa Mónica</h2><form id='fL'>
            <input name='u' placeholder='Usuario' style='margin-bottom:10px;'>
            ...
            <script>async function doLogin(){ const res=await fetch('/api/login',{method:'POST', body:new FormData(document.getElementById('fL'))}); const data=await res.json(); if(data.ok) location.href='/dashboard'; else alert('Error'); }</script>"""
    ...
    if method == "POST":
        ...
        if path == "/api/save_per":
            id_p = fs.getvalue("id")
            if id_p: cur.execute("UPDATE perfiles SET strNombrePerfil=%s, bitAdministrador=%s WHERE id=%s", (fs.getvalue("n"), fs.getvalue("a"), id_p))
            ...
            id_m = fs.getvalue("id")
            if id_m: cur.execute("UPDATE modulos SET strNombreModulo=%s, strMenuPadre=%s WHERE id=%s", (fs.getvalue("n"), fs.getvalue("p"), id_m))
        ...
        elif path == "/api/del_user": cur.execute("DELETE FROM usuarios WHERE id=%s", (fs.getvalue("id"),))
    ...
    if path == "/perfiles":
        ...
        content = f"""<div class='card'><div style='display:flex; justify-content:space-between;'><h2>Perfiles</h2><button class='btn-blue' onclick='document.getElementById("mPer").style.display="block"'>+ Nuevo Perfil</button></div><table><tr><th>ID</th><th>NOMBRE</th><th>ADMIN</th><th>ACCIONES</th></tr>{rows_h}</table></div>
        ...
        <script>const editPer=(id,n,a)=>{{document.getElementById('eid').value=id;document.getElementById('en').value=n;document.getElementById('ea').value=a;document.getElementById('mPer').style.display='block';}};
        ...
        async function delPer(id){{if(confirm('¿Eliminar?')){{const fd=new FormData();fd.append('id',id);await fetch('/api/del_per',{{method:'POST',body:fd}});location.reload();}}}}</script>"""
        ...
        content = f"""<div class='card'><div style='display:flex; justify-content:space-between;'><h2>Módulos</h2><button class='btn-blue' onclick='document.getElementById("mMod").style.display="block"'>+ Nuevo</button></div><table><tr><th>Nombre</th><th>Padre</th><th>Acciones</th></tr>{rows_h}</table></div>
        ...
        <script>const editMod=(id,n,p)=>{{document.getElementById('mid').value=id;document.getElementById('mn').value=n;document.getElementById('mp').value=p;document.getElementById('mMod').style.display='block';}};
        ...
        async function delMod(id){{if(confirm('¿Eliminar?')){{const fd=new FormData();fd.append('id',id);await fetch('/api/del_mod',{{method:'POST',body:fd}});location.reload();}}}}</script>"""

    elif path == "/usuarios":
        try:
            cur.execute("SELECT u.*, p.strNombrePerfil FROM usuarios u LEFT JOIN perfiles p ON u.idPerfil = p.id")
            usrs = cur.fetchall(); cur.execute("SELECT id, strNombrePerfil FROM perfiles"); perfs = cur.fetchall()
            p_opts = "".join([f"<option value='{p['id']}'>{p['strNombrePerfil']}</option>" for p in perfs])
            rows = "".join([f"<tr><td><img src='data:image/png;base64,{u['strImagen']}' style='width:35px;height:35px;border-radius:50%;' onerror='this.src=\"https://via.placeholder.com/35\"'></td><td>{u['strNombreUsuario']}</td><td>{u['strCorreo']}</td><td>{u.get('strNombrePerfil','-')}</td><td>{u['strEstado']}</td><td><button class='btn-red' onclick='delUsr({u['id']})'>Eliminar</button></td></tr>" for u in usrs])
            content = f"""<div class='card'><div style='display:flex; justify-content:space-between;'><h2>Usuarios</h2><button class='btn-blue' onclick='document.getElementById("mU").style.display="block"'>+ Nuevo Usuario</button></div><table><tr><th>IMG</th><th>USUARIO</th><th>CORREO</th><th>PERFIL</th><th>ESTADO</th><th>ACCIONES</th></tr>{rows}</table></div>
            <div id="mU" class="modal"><div class="modal-content" style="width:650px;"><h3>Nuevo Usuario</h3><form id="fU"><div style="display:grid; grid-template-columns:1fr 1fr; gap:15px;">
            <div><label>Usuario</label><input name="u" required></div><div><label>Correo</label><input name="e" type="email" required></div><div><label>Pass</label><input name="p" type="password" required></div><div><label>Celular</label><input name="c"></div><div><label>Perfil</label><select name="pid">{p_opts}</select></div><div><label>Estado</label><select name="est"><option>Activo</option><option>Inactivo</option></select></div>
            </div><label>Foto</label><input type="file" name="img"><button type="submit" class="btn-blue" style="width:100%; margin-top:15px;">Guardar</button></form></div></div>
        ...
        except Exception as e: content = f"<div class='card'><h2>Error Usuarios</h2><p>{str(e)}</p></div>"
        ...
        content = f"""<div class='card'><h2>Matriz de Permisos</h2><label>Seleccione Perfil:</label>
            <select id="selPer" style="width:300px; margin-bottom:20px;" onchange="document.getElementById('tP').style.display=this.value?'block':'none'">
            <option value="">-- Seleccionar --</option>{p_opts}</select>
            <div id="tP" style="display:none;"><table><tr><th>Módulo</th><th>C</th><th>A</th><th>E</th><th>D</th></tr>{m_rows}</table><button class='btn-blue' style='margin-top:20px;'>Guardar Permisos</button></div></div>"""

    elif path == "/logout":
        start_response("303 See Other", [("Location", "/login"), ("Set-Cookie", "token=; Path=/; Max-Age=0")]); return [b""]
    else:
        content = "<div class='card'><h1>Dashboard</h1><p>Seleccione una opción del menú de Seguridad.</p></div>"
