...
def render_layout(title, content, user=None):
    ...
    if user:
        ...
        # Lógica de seguridad para Admin (1) o usuarios con permisos
        if u_pid == 1:
        ...
        <div class="dropdown"><button class="dropbtn">Módulos ▾</button><div class="dropdown-content">{get_links("Principal 1") or '<a>(Vacío)</a>'}</div></div>
...
def application(environ, start_response):
    ...
    if path == "/api/crud" and method == "POST":
        ...
        try:
            if p['action'] == 'delete': cur.execute(f"DELETE FROM {p['table']} WHERE id=%s", (p['id'],))
                ...
                # FIX: Quitamos strCelular y strCorreo por si no existen en tu DB según el error 1054
    ...
    if path == "/usuarios":
        ...
        <div id="mU" class="modal"><div class="modal-content"><h3>Nuevo Usuario</h3>
        ...
        <button class="btn-emerald" onclick="runCrud('save_usuario','usuarios',0,{{u:document.getElementById('un').value, p:document.getElementById('up').value, idp:document.getElementById('uip').value, st:document.getElementById('ust').value, img:''}})">GUARDAR</button>
        <button onclick="closeM('mU')">Cerrar</button></div></div>"""
