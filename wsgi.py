...
def render_layout(title, content, user=None):
    ...
    if user:
        ...
        menu_html = ""
        # Lista de nombres que NO deben mostrarse en el menú desplegable porque ya son fijos
...
def application(environ, start_response):
    ...
    if path in ["/", "/login"]:
        content = """<div class="card" style="max-width:350px; margin:100px auto; text-align:center;">
            <h2 style="color:#38bdf8;">Acceso</h2>
            <form id="fL">
                <input type="text" name="u" placeholder="Usuario" style="width:100%; margin-bottom:15px;">
                <input type="password" name="p" placeholder="Contraseña" style="width:100%; margin-bottom:20px;">
    ...
    # --- RUTA PERFILES (REPARADA) ---
    if path == "/perfiles":
        conn = conectar_bd(); cur = conn.cursor(dictionary=True)
        cur.execute("SELECT * FROM perfiles")
        rows = cur.fetchall(); cur.close(); conn.close()
        rows_h = "".join([f"<tr><td>{r['id']}</td><td>{r['strNombrePerfil']}</td><td>{'SÍ' if r['bitAdministrador'] else 'NO'}</td><td><span style='color:#38bdf8;'>Editar</span></td></tr>" for r in rows])
        content = f"<div class='card'><h2>Gestión de Perfiles</h2><table><thead><tr><th>ID</th><th>Nombre Perfil</th><th>Administrador</th><th>Acciones</th></tr></thead><tbody>{rows_h}</tbody></table></div>"
    ...
    if path == "/permisos":
        ...
        if pid:
            ...
            inner = "<div style='text-align:center; padding:50px; color:#64748b;'><h3>⚠️ Selecciona un perfil para administrar permisos.</h3></div>"
    ...
    # --- RUTA MÓDULOS (CRUD ACTIVO) ---
    if path == "/modulos":
        conn = conectar_bd(); cur = conn.cursor(dictionary=True)
        cur.execute("SELECT * FROM modulos"); rows = cur.fetchall(); cur.close(); conn.close()
        rows_h = "".join([f"<tr><td>{r['strNombreModulo']}</td><td>{r.get('strMenuPadre','Seguridad')}</td><td style='color:#ef4444; cursor:pointer;'>Eliminar</td></tr>" for r in rows])
        ...
        <div id="mMod" class="modal"><div class="modal-content">
            ...
            <form id="fMod">
                <label>Nombre del Módulo *</label><input name="n" required placeholder="Ej: Reportes">
                <label style='display:block; margin-top:15px;'>Agrupar en Menú</label>
                <select name="p" style="width:100%"><option value="Seguridad">Seguridad</option><option value="Principal 1">Principal 1</option><option value="Principal 2">Principal 2</option></select>
                <div style="margin-top:25px; text-align:right;">
                    ...
                    <button class="btn-blue">Guardar Módulo</button>
                </div>
            </form>
        </div></div>"""
    ...
    start_response("200 OK", [("Content-Type", "text/html")]); return [render_layout("Dashboard", "<div class='card'><h1>Bienvenido al Panel</h1></div>", u_data).encode("utf-8")]