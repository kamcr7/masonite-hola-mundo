...
# =========================================================
# CONFIGURACIÓN (Mantenemos tu conexión de Railway)
# =========================================================
...
# =========================================================
# MAQUETACIÓN ESTILO ORIGINAL RESTAURADA
# =========================================================
...
def application(environ, start_response):
    ...
    # --- CRUD API CON SOPORTE PARA IMÁGENES ---
    if path == "/api/crud" and method == "POST":
        ...
        try:
            if p['action'] == 'delete': cur.execute(f"DELETE FROM {p['table']} WHERE id=%s", (p['id'],))
                ...
                if p['table'] == 'usuarios':
                    # Nota: He añadido strImagen si tu tabla la tiene, si falla por columna desconocida, quítalo
    ...
    if path == "/usuarios":
        ...
        rows = ""
        for u in usuarios:
            # FIX: Si no hay imagen, usamos un placeholder. Si hay, usamos el base64.
            img_src = u.get('strImagen') if u.get('strImagen') else "https://ui-avatars.com/api/?name=" + u['strNombreUsuario']
            rows += f"""<tr>
                <td><img src="{img_src}" class="avatar-table"></td>
        ...
        <div id='mNew' class='modal'><div class='modal-content'><span class='close-x' onclick="closeM('mNew')">&times;</span><h3>Nuevo Usuario</h3>
            ...
            <label>Foto de Perfil</label><input type='file' onchange="handleImg(event,'pv1','hi1')">
            <img id='pv1' style='width:60px;height:60px;border-radius:50%;object-fit:cover;margin:10px 0'><input type='hidden' id='hi1'>
            <button class='btn-emerald' style='width:100%' onclick=\"runCrud('save','usuarios',0,{{u:document.getElementById('un').value, p:document.getElementById('up').value, idp:document.getElementById('uip').value, st:document.getElementById('ust').value, img:document.getElementById('hi1').value}})\">GUARDAR USUARIO</button>
        </div></div>
        ...
        <div id='mEdit' class='modal'><div class='modal-content'><span class='close-x' onclick="closeM('mEdit')">&times;</span><h3>Editar Usuario</h3><input type='hidden' id='ed_id'>
            <input id='ed_u' placeholder='Usuario'><select id='ed_idp'>{p_opts}</select><select id='ed_st'><option>Activo</option><option>Inactivo</option></select>
            <label>Cambiar Foto</label><input type='file' onchange="handleImg(event,'pv2','hi2')">
            <img id='pv2' style='width:60px;height:60px;border-radius:50%;object-fit:cover;margin:10px 0'><input type='hidden' id='hi2'>
            <button class='btn-emerald' style='width:100%' onclick=\"runCrud('update','usuarios',document.getElementById('ed_id').value,{{u:document.getElementById('ed_u').value, idp:document.getElementById('ed_idp').value, st:document.getElementById('ed_st').value, img:document.getElementById('hi2').value}})\">ACTUALIZAR</button>
        </div></div>"""

    elif path == "/dashboard" or path == "/":
        content = f"<div class='card'><h2>Dashboard</h2><p>Bienvenido, <b>{u_data['u']}</b>.</p></div>"
    
    # ... (resto de rutas como /perfiles y /modulos igual que antes)
