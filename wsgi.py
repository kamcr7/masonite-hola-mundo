...
JWT_SECRET = "CLAVE_MAESTRA_CLINICA_2026_FINAL_V3"
RECAPTCHA_SITE_KEY = "TU_SITE_KEY_AQUI" # Reemplazar con tu llave de Google
...
def render_layout(title, content, user=None):
    ...
    if user:
        ...
        nav = f"""<div class="top-nav"><div class="nav-container"><div class="nav-left"><span class="logo">🏥 Clínica</span>
    ...
    <script>
        ...
        async function runCrud(action, table, id, data={{}}) {{
            ...
            if(res.ok) location.reload(); else alert("Error en la operación");
        }}
    </script>
...
def application(environ, start_response):
    ...
    if path == "/login":
        ...
        async function doLogin(){{
            ...
            if(d.ok) location.href="/dashboard"; else alert(d.msg || "Error");
        }}
        </script>"""
    ...
    if path == "/api/login" and method == "POST":
        ...
        # Aquí se validaría el captcha con Google (saltamos por demo)
        conn = conectar_bd(); cur = conn.cursor(dictionary=True)
        ...
        if user:
            ...
            return [b'{"ok":true}']
        start_response("200 OK", [("Content-Type", "application/json")]); return [b'{"ok":false, "msg":"Credenciales inválidas"}']
    ...
    if path == "/modulos":
        mods_fijos = [{'id': 'S', 'n': 'Perfiles', 'p': 'Seguridad'}, {'id': 'S', 'n': 'Módulos', 'p': 'Seguridad'}, {'id': 'S', 'n': 'Usuarios', 'p': 'Seguridad'}, {'id': 'S', 'n': 'Permisos', 'p': 'Seguridad'}]
        cur.execute("SELECT id, strNombreModulo as n, strMenuPadre as p FROM modulos"); mods_db = cur.fetchall()
        rows = "".join([f"<tr><td>{m['n']}</td><td>{m['p']}</td><td>{f'<button class=\"btn-red\" onclick=\"runCrud(\'delete\',\'modulos\',{m[\'id\']})\">Eliminar</button>' if m['id']!='S' else '<em>Sistema</em>'}</td></tr>" for m in mods_fijos + mods_db])
        content = f"""<div class='card'><h2>📦 Gestión de Módulos</h2><button class='btn-emerald' onclick="openM('mM')">+ Nuevo Módulo</button>
            ...
            <input id="mn" placeholder="Nombre"><input id="mr" placeholder="Ruta (/ejemplo)"><select id="mp"><option>Principal 1</option><option>Principal 2</option><option>Seguridad</option></select>
        ...
        content = f"""<div class='card'><h2>👤 Perfiles de Usuario</h2><button class='btn-emerald' onclick="openM('mP')">+ Nuevo Perfil</button>
            ...
            <input id="pn" placeholder="Nombre del Perfil (Ej: Administrador)">
            <button class="btn-emerald" style="width:100%" onclick="runCrud('save_perfil','perfiles',0,{{n:document.getElementById('pn').value}})">Guardar</button></div></div>"""

    elif path == "/usuarios":
        cur.execute("SELECT u.*, p.strNombrePerfil FROM usuarios u LEFT JOIN perfiles p ON u.idPerfil = p.id")
        rows = "".join([f"<tr><td>{u['strNombreUsuario']}</td><td>{u['strNombrePerfil']}</td><td>{u['strEstado']}</td><td><button class='btn-red' onclick=\"runCrud('delete','usuarios',{u['id']})\">Eliminar</button></td></tr>" for u in cur.fetchall()])
        ...
        content = f"""<div class='card'><h2>👥 Usuarios</h2><button class='btn-emerald' onclick="openM('mU')">+ Nuevo Usuario</button>
            <table><thead><tr><th>Usuario</th><th>Perfil</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{rows}</tbody></table></div>
            <div id="mU" class="modal"><div class="modal-content"><span class="close-x" onclick="closeM('mU')">&times;</span><h3>Nuevo Usuario</h3>
            <input id="un" placeholder="Username"><input id="up" type="password" placeholder="Password"><select id="uip">{perfs}</select>
            <button class="btn-emerald" style="width:100%" onclick="runCrud('save_usuario','usuarios',0,{{u:document.getElementById('un').value, p:document.getElementById('up').value, idp:document.getElementById('uip').value}})">Guardar</button></div></div>"""

    elif path == "/permisos":
        pid = int(urllib.parse.parse_qs(environ.get('QUERY_STRING','')).get('p',['0'])[0])
        cur.execute("SELECT * FROM perfiles"); perfs = cur.fetchall()
        table_html = "<p style='text-align:center; padding:20px; color:#94a3b8;'>Seleccione un perfil para gestionar permisos.</p>"
        if pid > 0:
            cur.execute("SELECT id, strNombreModulo as n FROM modulos"); all_m = [{'id':0, 'n':'Seguridad Base'}] + cur.fetchall()
            ...
            table_html = f"""<div style="display:flex; justify-content:flex-end; margin-bottom:10px;"><button class="btn-emerald" style="background:#334155" onclick="toggleAll()">Seleccionar Todo</button></div>
            <table><thead><tr><th>Módulo</th><th>Ver</th><th>Crear</th><th>Editar</th><th>Eliminar</th></tr></thead><tbody>{m_rows}</tbody></table>
            <button class="btn-emerald" style="width:100%; margin-top:20px">Guardar Configuración</button>"""
        opts = "".join([f"<option value='{p['id']}' {'selected' if p['id']==pid else ''}>{p['strNombrePerfil']}</option>" for p in perfs])
        content = f"<div class='card'><h2>🔐 Matriz de Permisos</h2><select onchange=\"location.href='?p='+this.value\"><option value='0'>-- Seleccione Perfil --</option>{opts}</select>{table_html}</div>"

    else:
        content = f"<div class='card'><h2>Bienvenido al Sistema</h2><p>Use el menú de Seguridad para configurar el sistema.</p></div>"
