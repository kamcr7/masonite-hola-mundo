...
def render_layout(title, content, user=None):
    ...
    if user:
        ...
        def get_menu(padre, extra_html=""):
            links = "".join([f'<a href="{m["strRuta"]}">📦 {m["strNombreModulo"]}</a>' for m in mods_db if m.get('strMenuPadre') == padre])
            return f'<div class="dropdown"><button class="dropbtn">{padre} ▾</button><div class="dropdown-content">{extra_html}{links}</div></div>'
        ...
        nav = f"""<div class="top-nav">
            <div class="nav-container">
                <div class="nav-left">
                    ...
                    {get_menu("Seguridad", seg_base)}
                    {get_menu("Principal 1")}
                    {get_menu("Principal 2")}
                </div>
    ...
    <style>
        ...
        .btn-outline {{ background:transparent; border:1px solid var(--border); color:#94a3b8; padding:6px 12px; border-radius:6px; cursor:pointer; text-decoration:none; font-size:0.8rem; }}
        ...
        .modal-content {{ background:var(--card); margin:8% auto; padding:25px; border:1px solid var(--border); width:400px; border-radius:16px; }}
    </style>
    <script>
...
def application(environ, start_response):
    ...
    if path in ["/", "/login"] and method == "GET":
        content = """<div class="card" style="width:350px; margin:100px auto; text-align:center;">
            <h2 style="color:var(--emerald)">Acceso Clínica</h2>
            <form id="fL">
    ...
    if path == "/api/crud" and method == "POST":
        ...
        try:
            if p['action'] == 'delete': cur.execute(f"DELETE FROM {p['table']} WHERE id=%s", (p['id'],))
                ...
                d = p['data']
                if p['id']: cur.execute("UPDATE usuarios SET strNombreUsuario=%s, strCorreo=%s, idPerfil=%s WHERE id=%s", (d['u'], d['c'], d['p'], p['id']))
                ...
                d = p['data']
                cur.execute("INSERT INTO modulos (strNombreModulo, strRuta, strMenuPadre) VALUES (%s,%s,%s)", (d['n'], d['r'], d['p']))
    ...
    if path == "/usuarios":
        ...
        content = f"""<div class='card'><h2>👥 Usuarios</h2><button class='btn-emerald' onclick="openM('mU')">+ Nuevo Usuario</button>
            ...
            <button class="btn-emerald" onclick="saveU()">Guardar</button><button onclick="closeM('mU')">Cancelar</button></div></div>
        ...
        m_rows = "".join([f"<tr data-mid='{m['id']}'><td>{m['strNombreModulo']}</td><td><input type='checkbox' class='c' {'checked' if perms.get(m['id'],{}).get('blnCrear') else ''}></td><td><input type='checkbox' class='e' {'checked' if perms.get(m['id'],{}).get('blnEditar') else ''}></td><td><input type='checkbox' class='v' {'checked' if perms.get(m['id'],{}).get('blnVer') else ''}></td></tr>" for m in mods])
        content = f"""<div class='card'><h2>🔐 Permisos</h2><select onchange="location.href='?p='+this.value"><option value="0">-- Selecciona Perfil --</option>{opts}</select>
            <button class="btn-emerald" onclick="saveP()">Guardar Permisos</button><table><thead><tr><th>Módulo</th><th>C</th><th>E</th><th>V</th></tr></thead><tbody>{m_rows}</tbody></table></div>
            <script>async function saveP(){{ const pid=new URLSearchParams(location.search).get('p'); const ps=[]; document.querySelectorAll('tbody tr').forEach(r=>ps.push([pid, r.dataset.mid, r.querySelector('.c').checked?1:0, r.querySelector('.e').checked?1:0, 0, r.querySelector('.v').checked?1:0])); await runCrud('save_permisos','permisos',0,{{idPerfil:pid, perms:ps}}); }}</script>"""
        ...
        content = f"<div class='card'><h2>👤 Perfiles</h2><button class='btn-emerald' onclick=\"openM('mP')\">+ Nuevo Perfil</button><table><thead><tr><th>ID</th><th>Perfil</th><th>Acción</th></tr></thead><tbody>{rows}</tbody></table></div><div id='mP' class='modal'><div class='modal-content'><h3>Nuevo Perfil</h3><input id='pn' placeholder='Nombre'><button class='btn-emerald' onclick=\"runCrud('save_perfil','perfiles',0,{{nombre:document.getElementById('pn').value}})\">Guardar</button></div></div>"

    elif path == "/logout": start_response("303 See Other", [("Location", "/login"), ("Set-Cookie", "token=; Max-Age=0")]); return [b""]
    else: content = f"<h2>Bienvenido {u_data['u']}</h2><p>Usa el menú superior para navegar.</p>"
