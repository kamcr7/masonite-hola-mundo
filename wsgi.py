...
def application(environ, start_response):
    ...
    if path == "/perfiles":
        ...
        content = f"""<div class='card'><h2>👤 Gestión de Perfiles</h2><button class='btn-emerald' style='width:auto' onclick="openM('mP')">+ NUEVO PERFIL</button>
        <table><thead><tr><th>ID</th><th>NOMBRE</th><th>ACCIONES</th></tr></thead><tbody>{rows}</tbody></table></div>
        <div id='mP' class='modal'><div class='modal-content'><span class='close-x' onclick="closeM('mP')">&times;</span>
        <h3>Nuevo Perfil</h3><input id='pn' placeholder='Nombre del Perfil'>
        <button class='btn-emerald' onclick=\"runCrud('save','perfiles',0,{{n:document.getElementById('pn').value}})\">GUARDAR</button></div></div>"""
    ...
    # --- RUTA MODULOS ---
        ...
        content = f"""<div class='card'><h2>📦 Gestión de Módulos</h2><button class='btn-emerald' style='width:auto' onclick="openM('mM')">+ NUEVO MÓDULO</button>
        <table><thead><tr><th>NOMBRE</th><th>RUTA</th><th>MENÚ PADRE</th><th>ACCIONES</th></tr></thead><tbody>{rows}</tbody></table></div>
        <div id='mM' class='modal'><div class='modal-content'><span class='close-x' onclick="closeM('mM')">&times;</span>
        <h3>Nuevo Módulo</h3><input id='mn' placeholder='Nombre'><input id='mr' placeholder='/ruta'><input id='mp' placeholder='Principal 1'>
        <button class='btn-emerald' onclick=\"runCrud('save','modulos',0,{{n:document.getElementById('mn').value, r:document.getElementById('mr').value, p:document.getElementById('mp').value}})\">GUARDAR</button></div></div>"""
    ...
    # --- RUTA USUARIOS ---
        ...
        content = f"<div class='card'><h2>👥 Usuarios</h2><button class='btn-emerald' style='width:auto' onclick=\"openM('mU')\">+ NUEVO</button><table><thead><tr><th>USUARIO</th><th>PERFIL</th><th>ESTADO</th><th>ACCIONES</th></tr></thead><tbody>{rows}</tbody></table></div>"
        content += f"<div id='mU' class='modal'><div class='modal-content'><span class='close-x' onclick=\"closeM('mU')\">&times;</span><h3>Nuevo Usuario</h3><input id='un' placeholder='User'><input id='up' type='password' placeholder='Pass'><select id='uip'>{p_opts}</select><button class='btn-emerald' onclick=\"runCrud('save','usuarios',0,{{u:document.getElementById('un').value, p:document.getElementById('up').value, idp:document.getElementById('uip').value, st:'Activo'}})\">GUARDAR</button></div></div>"
