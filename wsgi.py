...
def application(environ, start_response):
    ...
    # --- API CRUD CORREGIDA PARA IMÁGENES ---
    if path == "/api/crud" and method == "POST":
        ...
        try:
            if p['action'] == 'delete': cur.execute(f"DELETE FROM {p['table']} WHERE id=%s", (p['id'],))
                ...
                if p['table'] == 'usuarios':
                    sql = "UPDATE usuarios SET strNombreUsuario=%s, idPerfil=%s, strEstado=%s"
                    params = [p['data']['u'], p['data']['idp'], p['data']['st']]
                    if p['data'].get('img'): sql += ", strImagen=%s"; params.append(p['data']['img'])
                    sql += " WHERE id=%s"; params.append(p['id'])
    ...
    if path == "/usuarios":
        ...
        <div id='mNew' class='modal'><div class='modal-content'><span class='close-x' onclick="closeM('mNew')">&times;</span><h3>Nuevo Usuario</h3>
            <div class='grid-2'><div><label>User</label><input id='un'></div><div><label>Pass</label><input id='up' type='password'></div><div><label>Perfil</label><select id='uip'>{p_opts}</select></div><div><label>Estado</label><select id='ust'><option>Activo</option><option>Inactivo</option></select></div></div>
            <label>Foto</label><input type='file' onchange="handleImg(event,'pv1','hi1')"><img id='pv1' style='width:50px;border-radius:50%'><input type='hidden' id='hi1'>
            <button class='btn-emerald' style='width:100%' onclick=\"runCrud('save','usuarios',0,{{u:document.getElementById('un').value, p:document.getElementById('up').value, idp:document.getElementById('uip').value, st:document.getElementById('ust').value, img:document.getElementById('hi1').value}})\">GUARDAR</button></div></div>
        <div id='mEdit' class='modal'><div class='modal-content'><span class='close-x' onclick="closeM('mEdit')">&times;</span><h3>Editar Usuario</h3><input type='hidden' id='ed_id'><input id='ed_u'><select id='ed_idp'>{p_opts}</select><select id='ed_st'><option>Activo</option><option>Inactivo</option></select>
            <input type='file' onchange="handleImg(event,'pv2','hi2')"><img id='pv2' style='width:50px'><input type='hidden' id='hi2'>
            <button class='btn-emerald' style='width:100%' onclick=\"runCrud('update','usuarios',document.getElementById('ed_id').value,{{u:document.getElementById('ed_u').value, idp:document.getElementById('ed_idp').value, st:document.getElementById('ed_st').value, img:document.getElementById('hi2').value}})\">ACTUALIZAR</button></div></div>"""
    ...
    # --- VISTA PERFILES ---
        ...
        content = f"""<div class='card'><h2>👤 Perfiles</h2><button class='btn-emerald' onclick="openM('mNewP')">+ NUEVO PERFIL</button><table><thead><tr><th>ID</th><th>Nombre</th><th>Acciones</th></tr></thead><tbody>{rows}</tbody></table></div>
        <div id='mNewP' class='modal'><div class='modal-content'><span class='close-x' onclick="closeM('mNewP')">&times;</span><h3>Nuevo Perfil</h3><input id='pn' placeholder='Nombre del Perfil'><button class='btn-emerald' style='width:100%' onclick=\"runCrud('save','perfiles',0,{{n:document.getElementById('pn').value}})\">CREAR</button></div></div>
        <div id='mEditP' class='modal'><div class='modal-content'><span class='close-x' onclick="closeM('mEditP')">&times;</span><h3>Editar Perfil</h3><input type='hidden' id='ed_id'><input id='ed_n'><button class='btn-emerald' style='width:100%' onclick=\"runCrud('update','perfiles',document.getElementById('ed_id').value,{{n:document.getElementById('ed_n').value}})\">ACTUALIZAR</button></div></div>"""
    ...
    # --- VISTA MODULOS ---
        ...
        content = f"""<div class='card'><h2>📦 Módulos</h2><button class='btn-emerald' onclick="openM('mNewM')">+ NUEVO MÓDULO</button><table><thead><tr><th>Nombre</th><th>Ruta</th><th>Padre</th><th>Acciones</th></tr></thead><tbody>{rows}</tbody></table></div>
