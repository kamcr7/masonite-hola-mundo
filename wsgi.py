...
def application(environ, start_response):
    ...
    if path == "/usuarios":
        ...
        content = f"""<div class='card'><h2>Usuarios</h2>
        <table id='mT'><thead><tr><th>Usuario</th><th>Correo</th><th>Estado</th></tr></thead><tbody>{rows}</tbody></table>
        <div id='pag' class='pagination'></div></div>
        ...
        content = f"""<div class='card'>
            <div style='display:flex; justify-content:space-between;'><h2>Matriz de Permisos</h2><button onclick='setAll()' class='btn-blue'>✔️ Seleccionar Todos</button></div>
            <div id='tP'><table><thead><tr><th>Módulo</th><th>C</th><th>A</th><th>E</th><th>D</th></tr></thead><tbody>{m_rows}</tbody></table></div></div>"""
    
    elif path == "/logout":
        start_response("303 See Other", [("Location", "/login"), ("Set-Cookie", "token=; Path=/; Max-Age=0")]); return [b""]
        
    else:
        content = f"<div class='card'><h1>Dashboard</h1><p>Bienvenido al panel principal.</p></div>"
