...
def application(environ, start_response):
    ...
    if not u_data and path not in ["/", "/login"]:
    ...
    if path == "/usuarios":
        try:
            ...
            rows = ""
            for u in usrs:
                img_src = f"data:image/png;base64,{u['strImagen']}" if u.get('strImagen') else "https://via.placeholder.com/35"
                rows += f"<tr><td><img src='{img_src}' style='width:35px;height:35px;border-radius:50%;'></td><td>{u['strNombreUsuario']}</td><td>{u['strCorreo']}</td><td>{u.get('strNombrePerfil','-')}</td><td>{u['strEstado']}</td><td><button class='btn-red' onclick='delUsr({u['id']})'>Eliminar</button></td></tr>"
            ...
            <div id="mU" class="modal"><div class="modal-content" style="width:650px;"><h3>Nuevo Usuario</h3><form id="fU">
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px;">
                    <div><label>Usuario</label><input name="u" required></div><div><label>Correo</label><input name="e" type="email" required></div>
                    <div><label>Pass</label><input name="p" type="password" required></div><div><label>Celular</label><input name="c"></div>
                    <div><label>Perfil</label><select name="pid">{p_opts}</select></div><div><label>Estado</label><select name="est"><option>Activo</option><option>Inactivo</option></select></div>
                </div><label>Foto</label><input type="file" name="img"><button type="submit" class="btn-blue" style="width:100%;">Guardar</button></form></div></div>
            <script>
                ...
                async function delUsr(id){{ if(confirm('¿Eliminar?')){{ const fd=new FormData(); fd.append('id',id); await fetch('/api/del_user',{{method:'POST', body:fd}}); location.reload(); }} }}
            </script>"""
        except Exception as e:
            content = f"<div class='card'><h2>Error en Usuarios</h2><p>{str(e)}</p></div>"
        ...
        content = f"""<div class='card'><h2>Matriz de Permisos</h2>
            ...
            <select id="selPer" style="width:300px; margin-bottom:20px;" onchange="toggleTabla()">
                <option value="">-- Seleccionar --</option>{p_opts}
            </select>
            <div id="tablaPermisos" style="display:none;">
                <table><tr><th>Módulo</th><th>C</th><th>A</th><th>E</th><th>D</th></tr>{m_rows}</table>
                <button class='btn-blue' style='margin-top:20px;'>Guardar Permisos</button>
            </div></div>
            <script>
                function toggleTabla() {{
                    const val = document.getElementById('selPer').value;
                    document.getElementById('tablaPermisos').style.display = val ? 'block' : 'none';
                }}
            </script>"""

    elif path in ["/", "/login"]:
        content = """<div class='card' style='max-width:350px; margin:100px auto; text-align:center;'><h2>Login</h2><form id='fL'><input name='u' placeholder='Usuario'><input name='p' type='password' placeholder='Pass'><button type='button' onclick='doLogin()' class='btn-blue' style='width:100%; margin-top:20px;'>Entrar</button></form></div>
