...
def render_layout(title, content, user=None):
    ...
    if user:
        try:
            ...
            # Si la tabla está vacía, creamos un diccionario vacío en lugar de fallar
            mis_permisos = {p['nombreModulo']: p['permisoVer'] for p in perm_rows} if perm_rows else {}
            
            cur.close(); conn.close()
        except Exception as e:
            print(f"Error en BD Layout: {e}")
            mis_permisos = {}
            all_mods = []
        ...
        def get_links(padre):
            ...
            for m in all_mods:
                ...
                # Si no hay permisos en la BD, mis_permisos.get devuelve None (que no es 1)
        ...
        dropdown_seguridad = f"""
            ...
            </div>""" if html_seguridad or user['idPerfil'] == 1 else "" # El Admin siempre lo ve si quieres
...
def application(environ, start_response):
   ...
   # ----------------------------------------------------------
    # 7. USUARIOS (Sincronizado con tu tabla de permisos)
    # ----------------------------------------------------------
    elif path == "/usuarios":
        conn = conectar_bd()
        cur = conn.cursor(dictionary=True)

        # 1. Buscamos los permisos del perfil del usuario actual para este módulo
        # Usamos user['idPerfil'] que viene de la sesión
        cur.execute("""SELECT permisoCrear, permisoEditar, permisoEliminar 
                       FROM permisos 
                       WHERE idPerfil = %s AND nombreModulo = 'Usuarios'""", 
                    (user['idPerfil'],))
        p_user = cur.fetchone()

        # Si por alguna razón no hay permisos definidos para este perfil, 
        # denegamos por seguridad (puedes cambiar los 0 por 1 si prefieres que vean todo)
        if not p_user:
            p_user = {'permisoCrear': 0, 'permisoEditar': 0, 'permisoEliminar': 0}

        # 2. Tu consulta original de usuarios
        cur.execute("SELECT u.*, p.strNombrePerfil FROM usuarios u LEFT JOIN perfiles p ON u.idPerfil=p.id")
        usuarios = cur.fetchall()
        
        rows = ""
        for u in usuarios:
            # BOTONES CONDICIONALES: Solo se crean si el valor en la BD es 1
