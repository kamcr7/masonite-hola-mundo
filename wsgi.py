...
JWT_SECRET = "CLAVE_MAESTRA_CLINICA_2026_FINAL_V4"
RECAPTCHA_SITE_KEY = "6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI" # Key de prueba de Google
...
def render_layout(title, content, user=None):
    ...
    <style>
        ...
        table {{ width:100%; border-collapse:collapse; margin-top:20px; background:#0f172a; overflow:hidden; border-radius:8px; }}
        ...
        .btn-red {{ background:#ef4444; color:white; border:none; padding:8px 12px; border-radius:6px; cursor:pointer; font-size:12px; }}
        ...
        .modal-content input, .modal-content select {{ background:#f1f5f9; color:#1e293b; border-color:#cbd5e1; }}
        .close-x {{ position:absolute; top:15px; right:20px; color:#64748b; cursor:pointer; font-size:24px; font-weight:bold; }}
...
def application(environ, start_response):
    ...
    if path == "/login":
        content = f"""<div class="card" style="width:350px; margin:100px auto; border-top: 4px solid var(--emerald);">
            <h2 style="text-align:center; margin-bottom:30px;">Inicia Sesion</h2>
            <form id="fL">
    ...
    if path == "/api/login" and method == "POST":
        ...
        return [json.dumps({"ok":False, "msg":"Credenciales incorrectas"}).encode("utf-8")]
    ...
    if path == "/modulos":
        mods_fijos = [
            {'id':'S','n':'Perfiles','p':'Seguridad'}, {'id':'S','n':'Usuario','p':'Seguridad'},
            {'id':'S','n':'Permisos','p':'Seguridad'}, {'id':'S','n':'Principal 1','p':'Sistema'},
            {'id':'S','n':'Principal 2','p':'Sistema'}
        ]
        ...
        content = f"""<div class='card'><h2>📦 Gestion de Modulos</h2><button class='btn-emerald' onclick="openM('mM')">+ NUEVO MODULO</button>
            <table><thead><tr><th>Modulo</th><th>Agrupar en Menu</th><th>Acciones</th></tr></thead><tbody>{rows}</tbody></table></div>
            <div id="mM" class="modal"><div class="modal-content"><span class="close-x" onclick="closeM('mM')">&times;</span><h3>Nuevo Modulo</h3>
            <label>Nombre</label><input id="mn" placeholder="Nombre"><label>Ruta</label><input id="mr" placeholder="/ruta"><label>Menu</label>
            <select id="mp"><option>Principal 1</option><option>Principal 2</option><option>Seguridad</option></select>
        ...
        table_html = "<div style='text-align:center; padding:40px; color:#64748b;'>⚠️ Seleccione un perfil para ver la matriz de permisos.</div>"
        if pid > 0:
            cur.execute("SELECT id, strNombreModulo as n FROM modulos")
            all_m = [{'id':1, 'n':'Perfiles'},{'id':2,'n':'Usuario'},{'id':3,'n':'Permisos'},{'id':4,'n':'Principal 1'},{'id':5,'n':'Principal 2'}] + cur.fetchall()
            ...
            table_html = f"""<div style="display:flex; justify-content:flex-end; margin: 20px 0;"><button class="btn-emerald" style="background:#475569" onclick="toggleAll()">SELECCIONAR TODO</button></div>
        ...
        content = f"<div class='card'><h2>🔐 Matriz de Permisos</h2><label>Seleccionar Perfil:</label><select onchange=\"location.href='?p='+this.value\"><option value='0'>-- Elegir --</option>{opts}</select>{table_html}</div>"
        ...
        content = f"""<div class='card'><h2>👤 Perfiles</h2><button class='btn-emerald' onclick="openM('mP')">+ NUEVO PERFIL</button>
            <table><thead><tr><th>ID</th><th>Nombre Perfil</th><th>Accion</th></tr></thead><tbody>{rows}</tbody></table></div>
            <div id="mP" class="modal"><div class="modal-content"><span class="close-x" onclick="closeM('mP')">&times;</span><h3>Nuevo Perfil</h3>
            <input id="pn" placeholder="Nombre (Ej: Admin)"><button class="btn-emerald" style="width:100%" onclick="runCrud('save_perfil','perfiles',0,{{n:document.getElementById('pn').value}})">GUARDAR</button></div></div>"""
        ...
        content = f"""<div class='card'><h2>👥 Usuarios</h2><button class='btn-emerald' onclick="openM('mU')">+ NUEVO USUARIO</button>
            ...
            <button class="btn-emerald" style="width:100%" onclick="runCrud('save_usuario','usuarios',0,{{u:document.getElementById('un').value, p:document.getElementById('up').value, idp:document.getElementById('uip').value}})">CREAR USUARIO</button></div></div>"""

    elif path == "/logout":
        start_response("303 See Other", [("Location", "/login"), ("Set-Cookie", "token=; Max-Age=0; Path=/")]); return [b""]
    else:
        content = f"<div class='card'><h2>Dashboard</h2><p>Bienvenido, {u_data['u']}. Selecciona un modulo para empezar.</p></div>"
