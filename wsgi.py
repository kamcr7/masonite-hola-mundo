...
def application(environ, start_response):
    ...
    if path == "/usuarios":
        ...
        <input type="hidden" id="edit_id"><div class="grid-2">
        <input id="edit_u" placeholder="Usuario"><input id="edit_em" placeholder="Correo">
        <input id="edit_p" type="password" placeholder="Clave (Opcional)"><input id="edit_ph" placeholder="Celular">
        <select id="edit_idp">{p_opts}</select><select id="edit_st"><option>Activo</option><option>Inactivo</option></select>
        </div><input type="hidden" id="edit_img"><button class="btn-emerald" style="width:100%" onclick="runCrud('update_usuario','usuarios',document.getElementById('edit_id').value,{{u:document.getElementById('edit_u').value, em:document.getElementById('edit_em').value, p:document.getElementById('edit_p').value, ph:document.getElementById('edit_ph').value, idp:document.getElementById('edit_idp').value, st:document.getElementById('edit_st').value, img:document.getElementById('edit_img').value}})">ACTUALIZAR</button></div></div>"""

    elif path == "/modulos":
        content = "<div class='card'><h2>📦 Módulos</h2><p>Aquí puedes gestionar los módulos instalados.</p></div>"
    elif path == "/permisos":
        content = "<div class='card'><h2>🔐 Permisos</h2><p>Configuración de accesos por perfil de usuario.</p></div>"
    elif path == "/perfiles":
        cur.execute("SELECT id, strNombrePerfil as n FROM perfiles"); perfs = cur.fetchall()
        rows = "".join([f"<tr><td>{p['id']}</td><td>{p['n']}</td><td><button class='btn-red' onclick=\"runCrud('delete','perfiles',{p['id']})\">Borrar</button></td></tr>" for p in perfs])
